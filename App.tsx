    ...
    const filteredClients = useMemo(() => {
      return clientsWithTotals
        ...
        .sort((a, b) => {
          ...
          if (aP && !bP) return 1;
          if (!aP && bP) return -1;
          if (!aP && !bP) return a.nextDate - b.nextDate; // الأقرب سداداً
      ...
      <div className="pb-24 pt-4 px-4 h-full flex flex-col text-right">
        ...
        <div className="space-y-3 overflow-y-auto no-scrollbar pb-20">
          {filteredClients.map(client => (
            <div key={client.id} onClick={() => { setSelectedClientId(client.id); setSelectedDebtIds([]); setCurrentView('CLIENT_DETAILS'); }} className={`bg-white p-4 rounded-xl shadow-sm active:scale-[0.99] transition-all cursor-pointer ${client.remaining <= 0 ? 'opacity-50 grayscale border-dashed border-gray-200' : 'border border-transparent'}`}>
              <div className="flex justify-between items-start">
                <div><h3 className={`font-bold ${client.remaining <= 0 ? 'text-gray-400' : 'text-gray-800'}`}>{client.name}</h3><p className="text-xs text-gray-500 mt-1">{client.phone}</p>
             ...
             <div key={debt.id} className={`bg-white rounded-2xl shadow-sm overflow-hidden border transition-all ${selectedDebtIds.includes(debt.id) ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-100'}`}>
               <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-start">
                 <div className="flex items-center gap-3">
                   ...
                   <div><h4 className="font-bold text-gray-900">{debt.itemName}</h4>
                   <p className="text-xs text-gray-500 mt-1">أصل: {formatCurrency(debt.baseValue)} | ربح: {debt.profitPercentage.toFixed(1)}%</p></div>
                 </div>
            ...
            <div className="p-4 space-y-4">
                ...
                {balanceAfterThisPayment > 0 && (
                    <div className="bg-white p-5 rounded-2xl shadow-sm space-y-4 border-t-4 border-orange-400">
                        ...
                        <div className="flex items-center justify-between bg-gray-50 p-3 rounded-xl"><span className="text-sm font-medium">الأشهر القادمة</span><div className="flex items-center gap-4"><button onClick={() => setRemainingMonths(Math.max(1, remainingMonths - 1))} className="w-8 h-8 rounded-full bg-white">-</button><span className="font-bold">{remainingMonths}</span><button onClick={() => setRemainingMonths(remainingMonths + 1)} className="w-8 h-8 rounded-full bg-white">+</button></div></div>
                        <div className="divide-y">{previewInstallments.map((p, i) => (<div key={i} className="p-2 flex justify-between text-xs bg-white"><span>{i+1}</span><span className="font-bold">{formatCurrency(p.amount)}</span></div>))}</div>
                    </div>
                )}
                <button onClick={() => processPayment(debt.id, installment.id, paymentAmount, new Date(paymentDate).getTime(), notes, previewInstallments)} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold shadow-lg">{paymentAmount === 0 ? 'تأكيد التأجيل' : 'تأكيد السداد'}</button>
            </div>
        </div>
    );
  };
           ...
           <div className="bg-white p-4 rounded-xl shadow-sm space-y-4 text-right">
             ...
             <div className="bg-gray-50 p-3 rounded-xl border border-gray-200">
                <div className="flex bg-white rounded-lg p-1 shadow-sm mb-3">
                   ...
                   <button onClick={() => setProfitType('FIXED')} className={`flex-1 py-1.5 text-xs font-bold rounded-md flex items-center justify-center gap-1 ${profitType === 'FIXED' ? 'bg-blue-100 text-blue-700' : 'text-gray-500'}`}><Coins size={14} /> مبلغ ثابت</button>
                </div>
