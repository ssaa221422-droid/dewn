...
const [summaryPreview, setSummaryPreview] = useState<{ text: string; phone: string } | null>(null); // حالة النافذة المنبثقة الجديدة
 ...
 const ClientsListView = () => {
    ...
    return (
      <div className="pb-24 pt-4 px-4 h-full flex flex-col text-right">
        ...
        <div className="relative mb-6">
          <input type="text" placeholder="بحث باسم العميل أو الجوال..." className="w-full bg-white pl-4 pr-10 py-3 rounded-xl border-none shadow-sm text-sm focus:ring-2 focus:ring-blue-500 text-right" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
  ...
  const ClientDetailsView = () => {
    ...
    const handlePrepareSummary = () => {
      const debtsToSummarize = selectedDebtIds.length > 0 ? clientDebts.filter(d => selectedDebtIds.includes(d.id)) : clientDebts;
      if (debtsToSummarize.length === 0) { alert('يرجى اختيار مديونية واحدة على الأقل'); return; }
      ...
      <div className="pb-24 bg-gray-50 min-h-screen animate-fade-in text-right">
        <div className="bg-white pb-6 pt-4 px-4 rounded-b-3xl shadow-sm sticky top-0 z-10">
          ...
          <div className="flex flex-col items-center">
            ...
            <button onClick={handlePrepareSummary} className="mt-4 flex items-center gap-2 bg-green-50 text-green-700 px-6 py-2 rounded-xl font-bold"><Send size={16} /> كشف حساب</button>
          </div>
        </div>
        <div className="px-4 mt-6 space-y-6">
           <div className="flex justify-between items-center">
             <h3 className="font-bold text-gray-800">الديون المسجلة</h3>
             <button onClick={() => { setEditingDebtId(null); setCurrentView('ADD_DEBT'); }} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg">+ مديونية جديدة</button>
           </div>
             ...
             <div key={debt.id} className={`bg-white rounded-2xl shadow-sm overflow-hidden border transition-all ${selectedDebtIds.includes(debt.id) ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-100'}`}>
               <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-start">
                 <div className="flex items-center gap-3">
                   <input type="checkbox" checked={selectedDebtIds.includes(debt.id)} onChange={() => toggleDebtSelection(debt.id)} className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer" />
                   <div className="text-right">
                 ...
                 {debt.installments.map((inst, idx) => (
                   <div key={inst.id} className="p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                     ...
                     {inst.status === InstallmentStatus.PAID || inst.status === InstallmentStatus.POSTPONED ? (
                       <div className="flex items-center gap-2">
                         {inst.status === InstallmentStatus.PAID && (<button onClick={() => sendInstallmentReceipt(debt, inst, idx)} className="text-green-600 bg-green-50 p-1.5 rounded-md"><Receipt size={16} /></button>)}
               ...
               <div className="p-3 bg-gray-50 border-t text-center text-sm"><div className="flex justify-between items-center"><span className="text-gray-500">الإجمالي:</span><span className="font-bold text-gray-800">{formatCurrency(debt.totalValue)}</span></div></div>
             </div>
           ))}
        </div>
      </div>
  ...
  const RecordPaymentView = () => {
    ...
    return (
        <div className="bg-gray-50 min-h-screen pb-20 text-right">
            ...
            <div className="p-4 space-y-4">
                ...
                {balanceAfterThisPayment > 0 && (
                    <div className="bg-white p-5 rounded-2xl shadow-sm space-y-4 border-t-4 border-orange-400">
                        ...
                        <div className="divide-y">{previewInstallments.map((inst, idx) => (<div key={idx} className="px-4 py-2 flex justify-between items-center text-sm bg-white"><span>{idx + 1}</span><span className="font-bold">{formatCurrency(inst.amount)}</span><span>{formatDate(inst.dueDate)}</span></div>))}</div>
                    </div>
                )}
  ...
  const AddClientView = () => {
    ...
    return (
      <div className="bg-white min-h-screen pb-20 text-right">
  ...
  const DebtFormView = () => {
    ...
    const handleRecalculate = () => {
        const total = Number(baseValue) + (Number(baseValue) * (profitPercentage / 100));
    ...
    return (
      <div className="bg-gray-50 min-h-screen pb-24 text-right">
        <div className="bg-white p-4 border-b flex items-center"><button onClick={() => setCurrentView('CLIENT_DETAILS')} className="p-2 -mr-2 text-gray-600"><ArrowLeft /></button><h2 className="text-xl font-bold mr-2">{isEditMode ? 'تعديل' : 'إضافة'} مديونية</h2></div>
        <div className="p-4 space-y-4">
           <div className="bg-white p-4 rounded-xl shadow-sm space-y-4">
             ...
             <div><label className="block text-xs text-gray-500 mb-1">رأس المال</label><input type="number" value={baseValue} onChange={e => setBaseValue(Number(e.target.value))} className="w-full p-3 bg-gray-50 rounded-lg border font-bold text-right" /></div>
             <div className="grid grid-cols-2 gap-4"><div><label className="block text-xs text-gray-500 mb-1">الربح %</label><input type="number" value={profitPercentage} onChange={e => setProfitPercentage(Number(e.target.value))} className="w-full p-3 bg-gray-50 rounded-lg border text-right" /></div><div><label className="block text-xs text-gray-500 mb-1">الأشهر</label><input type="number" value={months} onChange={e => setMonths(Number(e.target.value))} className="w-full p-3 bg-gray-50 rounded-lg border text-right" /></div></div>
             <button onClick={handleRecalculate} className="w-full py-2 bg-gray-100 text-gray-700 font-bold rounded-lg border">إنشاء الجدول</button>
           </div>
           <button onClick={() => saveDebt({ id: editingDebtId, clientId: selectedClientId, itemName, baseValue, profitPercentage, monthCount: months, startDate, paymentDay, installments: manualInstallments }, isEditMode)} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold shadow-lg shadow-blue-200">حفظ المديونية</button>
        </div>
      </div>
    );
  };
      ...
      {summaryPreview && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl">
            ...
            <div className="bg-gray-50 p-4 rounded-2xl text-sm whitespace-pre-wrap max-h-[40vh] overflow-y-auto mb-6 border border-gray-100 leading-relaxed font-tajawal text-right">{summaryPreview.text}</div>
